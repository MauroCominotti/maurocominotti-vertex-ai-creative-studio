...
import { EnrichedSourceAsset, AspectRatioEnum, StyleEnum } from '../fun-templates/media-template.model';
...
fdescribe('VideoComponent', () => {
  ...
  beforeEach(async () => {
    mockSearchService = jasmine.createSpyObj('SearchService', ['startVeoGeneration', 'concatenateVideos', 'rewritePrompt', 'getRandomPrompt', 'clearActiveVideoJob'], { activeVideoJob$: of(null), videoPrompt: '' });
    ...
    mockVideoStateService = jasmine.createSpyObj('VideoStateService', ['getState', 'updateState', 'resetState']);
    mockMatIconRegistry = jasmine.createSpyObj('MatIconRegistry', ['addSvgIcon', 'getNamedSvgIcon']);
    ...
    (mockMatIconRegistry.getNamedSvgIcon as jasmine.Spy).and.returnValue(of(document.createElementNS('http://www.w3.org/2000/svg', 'svg')));
  ...
  describe('Initialization', () => {
    it('should have default searchRequest values', () => {
      ...
      expect(component.searchRequest.generationModel).toBe('veo-3.1-generate-preview');
  ...
  describe('State Management', () => {
    it('should save state', () => {
        ...
        component.saveState();
        expect(mockVideoStateService.updateState).toHaveBeenCalledWith(jasmine.objectContaining({
            prompt: 'test prompt'
        }));
    });
  ...
  describe('User Input', () => {
    ...
    it('should add a negative phrase', () => {
        const event = { value: ' blurry ', chipInput: { clear: () => {} } } as MatChipInputEvent;
  ...
  describe('Search Term', () => {
    beforeEach(() => {
        mockWorkspaceStateService.getActiveWorkspaceId.and.returnValue('test-workspace');
    });
    ...
    it('should not search if prompt is empty and not in extension mode', () => {
        ...
        expect(mockSnackBar.open).toHaveBeenCalledWith('Please enter a prompt to generate a video.', 'OK', jasmine.any(Object));
    });
...
it('should call startVeoGeneration on search', () => {
  ...
  expect(mockSearchService.startVeoGeneration).toHaveBeenCalled();
    ...
    it('should show snackbar if less than two videos for concatenation', () => {
        component.isConcatenateMode = true;
        component.sourceMediaItems = [{ mediaItemId: 1, mediaIndex: 0, role: 'video_source' }, null];
        component.searchTerm();
        expect(mockSnackBar.open).toHaveBeenCalledWith('Please select at least two videos to concatenate.', 'OK', { duration: 5000 });
    ...
    it('should switch to Veo 3.1 if source assets are present with Veo 3.0', () => {
        ...
        expect(mockSnackBar.open).toHaveBeenCalledWith("Veo 3 doesn't support images as input, so we've switched to Veo 3.1 for you.", 'OK', jasmine.any(Object));
    });
    ...
    it('should build payload correctly for Frames to Video mode', () => {
        ...
        expect(mockSearchService.startVeoGeneration).toHaveBeenCalledWith(expectedPayload);
    });
  ...
  describe('File Input and Clearing', () => {
    ...
    it('should open image selector and process video result', () => {
      const dialogRefSpyObj = jasmine.createSpyObj({ afterClosed: of({
        mediaItem: { id: 1, mimeType: 'video/mp4', presignedThumbnailUrls: ['thumb-url'] },
        selectedIndex: 0
      }), close: null });
      mockDialog.open.and.returnValue(dialogRefSpyObj);
      ...
      expect(component['_input1IsVideo']).toBeTrue();
      expect(component.sourceMediaItems[0]).toEqual({
        mediaItemId: 1,
        mediaIndex: 0,
    ...
    it('should move video from slot 2 to 1 if slot 1 is cleared', () => {
      ...
      expect(component['_input2IsVideo']).toBeFalse();
  ...
  describe('Reference Images', () => {
    it('should open image selector and add a reference image', () => {
        ...
        } as SourceAssetResponseDto), close: null });
        mockDialog.open.and.returnValue(dialogRefSpyObj);
    ...
    it('should clear other inputs and switch model when first reference image is added', () => {
        ...
        expect(mockSnackBar.open).toHaveBeenCalledWith('Start/end frames and extension videos have been cleared to use reference images.', 'OK', { duration: 5000 });
        expect(component.selectModel).toHaveBeenCalled();
        expect(mockSnackBar.open).toHaveBeenCalledWith("We've switched to the Veo 3.1 model for you, as this one supports reference images.", 'OK', jasmine.any(Object));
    });
  ...
  describe('Remix and Template Logic', () => {
    ...
    it('should apply source assets from navigation', () => {
      const sourceAssets = [
        { assetId: 10, gcsUri: 'gs://bucket/image.png', presignedUrl: 'url1', role: 'input' },
        { assetId: 11, gcsUri: 'gs://bucket/ref.png', presignedUrl: 'url2', role: 'image_reference_asset' }
