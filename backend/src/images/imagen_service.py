...
def _process_vto_in_background(
    ...
    try:
        ...
        logger.info(f"Starting VTO worker for media_id: {media_item_id}")
...
) -> tuple[Optional[types.GeneratedImage], Optional[dict]]:
    """
    Generates an image using the Gemini API for text-to-image or image-to-image.
    This is a blocking function.

    Returns:
        A tuple of (types.GeneratedImage, grounding_metadata), or (None, None) if failed.
    """
            ...
            if reference_images:
                for img in reference_images:
                    if img.gcs_uri:
                        parts.append(
                            types.Part.from_uri(
                                file_uri=img.gcs_uri, mime_type=img.mime_type or "image/png"
                            )
                        )
                    elif img.image_bytes:
                         parts.append(
                            types.Part.from_bytes(
                                data=img.image_bytes, mime_type=img.mime_type or "image/png"
                            )
                        )
        ...
        async def _async_worker():
            async with WorkerDatabase() as db_factory:
                async with db_factory() as db:
                    ...
                    # Add raw GCS URIs if provided (e.g. from Enforcer Agent) - Merged from HEAD
                    if request_dto.reference_image_gcs_uris:
                        for uri in request_dto.reference_image_gcs_uris:
                            reference_images_for_api.append(
                                types.Image(gcs_uri=uri, mime_type="image/png") # Defaulting to png
                            )
                    ...
                    try:
                        ...
                        # --- PATH 2: IMAGE EDITING (IMAGE-TO-IMAGE) ---
                            ...
                            if (
                                ...
                                # Fallback logic from HEAD: Download bytes if needed for reliable editing
                                if ref_img_obj.gcs_uri:
                                    try:
                                        worker_logger.info(f"Checking access to reference image: {ref_img_obj.gcs_uri}")
                                        pass # SDK handles it usually, but if we wanted to enforce bytes we could do it here.
                                    except Exception as e:
                                        worker_logger.warning(f"Error checking ref image: {e}")
